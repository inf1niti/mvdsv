...
void SV_RunCmd (usercmd_t *ucmd, qbool inside, qbool second_attempt) //bliP: 24/9
{
	...
	pmove.hook_initial_length = sv_client->hook_initial_length;
	pmove.hook_initial_radial_speed = sv_client->hook_initial_radial_speed;
	pmove.hook_initial_tangential_speed = sv_client->hook_initial_tangential_speed;
	...
	pmove.hook_back_blend = sv_client->hook_back_blend;
	pmove.hook_forward_blend = sv_client->hook_forward_blend;
#endif
...
FIXME
	...
	sv_client->hook_back_blend = pmove.hook_back_blend;
	sv_client->hook_forward_blend = pmove.hook_forward_blend;
#endif
...
static void SV_ServerSideWeaponLogic_PrePostThink(client_t* sv_client, ssw_info_t* ssw)
{
	...
	if ((sv_client->mvdprotocolextensions1 & MVD_PEXT1_SERVERSIDEWEAPON) && sv_client->weaponswitch_enabled) {
		...
		ssw->hiding = (ent->weapon != IT_HOOK && sv_client->weaponswitch_wasfiring && !ssw->firing && hide_impulse);
